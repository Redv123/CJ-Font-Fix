...
`PageLanguageDetector` first checks a hostname override (`off`, SC, TC, or JP). With `trustCjkLang` enabled, a specific root CJK `lang` then decides the page without browser detection. `FallbackController` also skips all element scanning and font injection for that trusted root, including dynamically added content and nested `lang` declarations. Bare `zh` lacks a script, so SC/TC clues and the configured default Chinese variant decide it. Otherwise the detector samples the title plus up to 10,000 characters from `main`, `[role=main]`, a single `article`, the nearest shared container of the first two articles, or `body`, as applicable. The shared container prevents a first advertisement card from hiding later search results. It excludes code-like and non-prose elements, rejects pages without CJK, and keeps surrounding Latin text for `chrome.i18n.detectLanguage()`. It does not filter text by computed visibility or remove duplicate text from this sample. Kana and Chinese clues are fallbacks when the browser result is insufficient. When the browser ranks Chinese just above Japanese by at most ten percentage points, a conservative check can select JP if multiple distinct Japanese segments have sufficient kana and Japanese-form Han evidence and the remaining segments lack strong Chinese evidence. This check does not override a trusted root declaration. A cached sample result avoids repeated asynchronous detection when the sample is unchanged.

On pages without a trusted specific root declaration, the page result is **not** the final choice for every element. In advanced mode, `ElementLanguage` checks inherited local `lang`; with the trust setting on, only a declaration mapping explicitly to SC, TC, or JP is left to the browser unless the user forced a site override. English and other unsupported tags, as well as bare `zh`, do not skip CJK font repair. For an element that still needs intervention, precedence is: forced site variant, explicit local variant, experimental strong local variant, then page variant. Mixed-language handling does not rewrite `lang` and does not change the page-level result. A manual site override takes precedence over automatic and declared results.
...
With dynamic detection enabled, an in-document browser Back/Forward (`popstate`) also requests one full rescan and language re-detection. A SPA can restore existing nodes without changing their text, so DOM mutations alone cannot reliably invalidate a dormant result. The request enters the normal coalesced scheduler; it does not start polling or run detection synchronously inside the navigation event.
