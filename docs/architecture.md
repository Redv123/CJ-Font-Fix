...
`.github/workflows/release.yml` builds from a GitHub source revision, checks that the package and manifest versions agree, requires a hand-written `docs/releases/v<version>.md`, and packages only `dist` as the Release asset. It runs unit tests and TypeScript checks, not live-site or optional corpus tests. Manual runs from `main` create the matching version tag after a successful build; matching tag pushes can trigger the same pipeline.
